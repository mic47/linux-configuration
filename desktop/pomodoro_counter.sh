...
WORKDAY_HOURS=7
WORKDAY_SECONDS=$((3600*$WORKDAY_HOURS))
...
echo "Time left $(hours_time "$(time_left Merlon "$TODAY_REGEX")")($(success_time Merlon "$TODAY_REGEX")%)"
echo "w: $(hours_time "$(time_left Merlon "$THIS_WEEK_REGEX")")($(success_time Merlon "$THIS_WEEK_REGEX")%)"
echo "m: $(days_time "$(time_left Merlon "$THIS_MONTH_REGEX" "$THIS_WORK_MONTH_REGEX")")($(success_time Merlon "$THIS_MONTH_REGEX")%)"
if [ -n "$2" ] ; then
  echo "Working days worked: $(working_days_worked Merlon "$THIS_WORK_MONTH_REGEX")"
  worked=$(worked_time "Merlon-oncall" "$THIS_MONTH_REGEX")
  if [ -z "$worked" ]; then
    worked="0"
  fi
  echo "Oncall emergency time $(hours_time "$worked") ($(hours_time_decimal "$worked"))"
  logs "Merlon\-oncall" | grep -e "$THIS_MONTH_REGEX"
fi
